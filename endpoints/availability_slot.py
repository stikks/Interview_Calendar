...
def retrieve(obj_id):
    """
    retrieve availability slots matching obj_id
    :param obj_id
    :return
    """
    ...
    slots = models.AvailabilitySlot.query.filter_by(interviewer_id=interviewer.id,
                                                    is_booked=False).offset(page).limit(limit).all()

    return jsonify({"data": [i.serialize() for i in slots]})
...
def create(obj_id, **kwargs):
    ...
    if form.validate_on_submit():
        if models.AvailabilitySlot.query.filter_by(scheduled_date=form.scheduled_date.data,
        ...
        return jsonify({"message": "Availability slot on date - {} scheduled for {} - "
...
def create_range(obj_id, **kwargs):
    """
    creates new interviewer availability record for date range
    :param obj_id:
    :param kwargs:
    :return:
    """
    ...
    if form.validate_on_submit():
        ...
        for i in range(0, period.days + 1):
            ...
            if models.AvailabilitySlot.query.filter_by(scheduled_date=select_date, start_time=form.start_time.data,
            ...
            models.db.session.add(slot)
            models.db.session.commit()
        return jsonify({"message": "Availability slot dates created"})
...
def delete(obj_id):
    """
    deletes availability slot record matching obj_id
    :param obj_id:
    :return:
    """
    slot = models.AvailabilitySlot.query.get(obj_id)

    if not slot:
        raise InvalidUsage("Availability slot not found matching id - %s" % obj_id, status_code=404)

    models.db.session.delete(slot)
    models.db.session.commit()

    return success_response("Interviewer successfully deleted", 204)
